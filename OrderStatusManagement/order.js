...
const executeOrder = () => {
	...
	chefReceived(orderIdInput)
		...
		.then(firstlayerofBiryaniRiceadded)
		.then(toppingsAdded)
		.then(keptforDum)
		.then(biryaniCooked)
...
const firstlayerofBiryaniRiceadded = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'First layer of Biryani Rice added'
			resolve(orderNumber)
		}, 5000)
	})
}
...
const keptforDum = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Kept for Dum '
			resolve(orderNumber)
		}, 5000)
	})
}
...
const createNewOrderColumn = (orderIdInput) => {
	...
	let cardTextPText = document.createTextNode('Dum Biryani Order Placed')
